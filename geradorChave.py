...
def generateKey(keySize):
    ...
    while p == q:
        p = geradorNumPrimo.generateLargePrime(keySize)
        q = geradorNumPrimo.generateLargePrime(keySize)
    n = p * q
    ...
    while True:
        ...
        if aritmetica.gcd(e, (p - 1) * (q - 1)) == 1:
            break

    # Step 3: Calculate d, the mod inverse of e:
    print('Calculating d that is mod inverse of e...')
    d = aritmetica.findModInverse(e, (p - 1) * (q - 1))
...
def makeKeyFiles(name, keySize):
    ...
    if os.path.exists('%s_pubkey.txt' % (name)) or os.path.exists('%s_privkey.txt' % (name)):
        sys.exit(
            'WARNING: The file %s_pubkey.txt or %s_privkey.txt already exists! Use a different name or delete these files and rerun this program.' % (name, name))
